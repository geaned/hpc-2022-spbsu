...
Пусть, как в `std::memory_order` из C++, нам доступны порядки `acquire`, `release`, `acq_rel` и `seq_cst` (есть еще `relaxed` и `consume`, но их в расчет не берем). Как бы мы применили их в нашем случае относительно уже атомарной переменной `b`, чтобы получить максимально производительный код?
